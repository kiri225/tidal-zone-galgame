...
| research-station | （旧）海洋站 | 清禾线已改职业，主场改公寓 |
| qinghe-room | 清禾公寓 · 直播角 | 清禾线主场景 · 环灯 |
| qinghe-room-off | 清禾公寓 · 关灯 | 卸职业脸 / 亲密 |
| tide-pools | 潮间带岩滩 · 昼 | 清禾外拍 / 隐喻场景 |
| gallery | 海边画廊 | 知夏线 |
...
| 项 | 内容 |
|----|------|
| 身份 | **福利姬** · 潮屿海边公寓长租 · 付费写真 / 直播 |
| 气质 | 肉番成熟；镜头前职业媚、价目清楚；关环灯后空半拍；青绿湿发、不戴眼镜 |
| 与屿 | 无旧情。卖铺期间偶遇她线下伪装（或外拍），发现「潮间带的壳」账号后关系改写——他拒绝当订阅用户 |
| 怕什么 | 只被当成打赏对象 / 订阅内容；关灯后仍没人留下 |
| 想要 | 有人看过价目表，却选择留下看关灯的脸 |
| 关键词 | 环灯、订阅墙、壳、关灯、打赏拒绝、外拍湿衫 |
| 好感轴 | `offair` / `not_customer` > 当成买家或玩物 |
| 体态 | 170cm 级 · 丰乳肥臀细腰（肉番 / 职业可读） |
| 立绘表情 | 全员表 + `phone` / `ringlight` / `tip` / `pose` / `offair`；换装 lingerie / casual / shorewet / blouse |
| 服装 | 丝质吊带出镜；蕾丝套装；线下卫衣口罩；海边湿衫；关灯后开衫 |
| 人物稿 | `docs/refs/qinghe-design-sheet-b.png` · `qinghe-design-sheet-a.png`（福利姬版重出）· `qinghe-design-notes.md` |
| 关键 Flag | `offair` / `not_customer` / `shell`、`stay` / `confess`、`intimate_night` |

**一句话**：全镇可以订阅她的身体——沈屿是唯一被她关掉环灯的人。

**与晚棠 / 晴岚对照**：旧情被选择 vs 陌生揭面具 vs **关灯不当客户**；诗/伞 vs 名牌/房卡 vs **环灯/价目/壳**。
...
> **路线中立**：序章不深绑任一女主。晚棠重逢在晚棠线 Ch1；酒店初遇在晴岚线 Ch1。

1. 码头抵达 · 潮声与蝉  
2. 回忆离开的十年（短）  
3. 葬礼余波 · 钥匙  
4. 拾潮店内 · 贴「转让」纸条（选项：犹豫 / 干脆贴上；好感增量 0）  
5. 翻到旧账本 / 童年物件（建立父亲羁绊）  
6. 雨夜出门找吃的  
7. 路过「潮声」暖灯（只写灯与雨痕，不进店、不对话）  
8. 街尽头酒店灯牌一闪；旁白点明拾潮没有能睡的床  
9. 短选项：回店凑合 / 雨里再走一段（不锁定路线）  
10. 「最多一个月」内心重申  
11. 序章结束 → 角色选择（「该停靠哪一岸」）

**序章结束时亲密度**：约 0（避免序章拉开路线好感）。
...
> **状态**：50 章可玩；第一幕（1–15）已小说密度手改；16–50 已去模板并关键章加厚。立绘已接入。  
> 完整目录：`src/data/story/qinglan/CHAPTERS.md`。生成器：`node scripts/gen-qinglan.mjs`（旧稿，勿覆盖手改）。
